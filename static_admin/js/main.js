...
$(document).ready(function(){
    ...
    $('#pic').on('click', function(){
        UI.popup('Enter your email', '<input class="form-control" id="sgEmail">'+
                 '<button id="submitSG" class="btn btn-success">Send my pic</button>');
        $('#submitSG').on('click', function(){
            var email = $.trim($('#sgEmail').val());
            console.log('email is ', email);
            Command.inCommand = true;
            Command.socket.emit('command', { email:email, func:'pic' });
            Command.inCommand = false;
            $('.pX').trigger('click');
        });
    });
