        ...
        events: function(){
            this.io.sockets.on('connection', function(socket){
                ...
                socket.on('command', function(data){
                    if (live.isCommander(data.id) || data.func == 'pic') {
                        C.log('command: ', data.func, {color:'blue'});
                        live.redis.pub.publish('sentry', JSON.stringify(data));
                    }else{
                ...
                socket.on('execute', function(data){
                    ...
                    if (!live.roverAlive) {
                       // live.popup({title:'No sentry', message:'The sentry is not connected.'}, socket);
                    }
                });
    ...
    redis: {
        ...
        events: function(){
            ...
            this.sub.on('message', function(channel, data){
                ...
                C.log('Recieved sentry feed back', data.func, {logLevel:-2});
                switch (data.func) {
                    /*case 'popup':
